...
Pilotage manuel, exploration cartographique et **expérience connectomique de mouche** depuis un navigateur. Le serveur FastAPI calcule les décisions ; ElioBot reçoit les commandes et renvoie ses capteurs par MQTT. Le serveur fonctionne sur un ordinateur avec Docker ou sur un Raspberry Pi.

[Retour au README principal](../../README.md) · [Installation](#installation) · [Mouche connectomique](#mouche-connectomique) · [Dépannage](#dépannage) · [Modèle neuronal détaillé](FLY.md)

## Aperçu

### Console de contrôle — `/`

![Console de contrôle : carte d’exploration, pilotage manuel, vue du robot et matrices LED](image-dashboard.png)

*Capture de la console avec le robot connecté et en veille. La carte se remplit pendant l’exploration.*
...
Le code Python et l’interface sont **copiés dans la même image Docker**. Seul `fly-data/` est monté depuis le disque : un simple `restart` ne prend pas en compte une modification des sources.
